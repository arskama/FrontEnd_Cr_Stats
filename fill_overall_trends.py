...
if (arguments != 1):
    ...
    print "Usage: python fill_overal_trends.py <filename>"
    sys.exit(1);
...
##############################################
def getValue(lastCompany, Linelist):
    cnt = 0;
    out = [];
    ret = "";
    val = int(0);
    ...
    for line in lineList:
        # Get start
        # Get real
        if (line.find(lastCompany) == 0):
            if (lastCompany == "ibm.com"):
                val += int(line.rpartition(",")[2]);
            if (lastCompany == "google.com"):
                val += int(line.rpartition(",")[2]);
...
filename = "/home/amandy/Statistics/FrontEnd_Cr_Stats/Data/Trends/ChromeOS.csv.bk";
print(filename);
...
for line in trendLine:
    ...
    if ((company != line.split(",")[0]) or cnt == len(trendLine)- 1):
        ...
        if (cnt == 1):
            ...
            ret = getValue(lastCompany, lineList);
            index = acc + last
...
result_file = open("/home/amandy/Statistics/FrontEnd_Cr_Stats/Data/Trends/ChromeOS.csv",'wb')
